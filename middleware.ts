...
export const config = {
  ...
  // users to /overview itself. "docs" is the public self-hosting guide —
  // no auth check needed there at all, it has no user data. Every other
  // route stays gated here.
  matcher: ["/((?!api/auth|api/plaid/webhook|login|docs|_next/static|_next/image|favicon.ico|$).*)"],
};
