...
//
// Lives under (tabs)/ like the 4 real tabs -- same header-less, in-content-
// title treatment as Budgets/Accounts -- but isn't declared as a
// NativeTabs.Trigger/Tabs.Screen, so it never shows as a 5th tab button.
// Reached only by pushing "/(tabs)/investments" from the More sheet. This
// replaced an earlier version living in the root Stack with a native
// header: a custom headerRight view there picked up iOS 26's automatic
// "Liquid Glass" bar-button chrome around the Sync action, which a
// header-less (tabs)/ screen with its own in-content back button sidesteps
// entirely.
  ...
  const header = (
    <View className="flex-row items-center justify-between px-5 pb-4">
      <View className="flex-row items-center gap-2.5 flex-1 pr-2">
        <Pressable onPress={() => router.back()} hitSlop={12} className="items-center justify-center rounded-full bg-surface-2" style={{ width: 32, height: 32 }}>
          <ChevronLeft size={20} color={colors.text} strokeWidth={2} />
        </Pressable>
        <Text className="font-ui-semibold text-text" style={{ letterSpacing: -0.3, fontSize: rf(22) }} numberOfLines={1}>
          Investments
        </Text>
      </View>
      <Pressable
  ...
  return (
    <View className="flex-1 bg-canvas" style={{ paddingTop: insets.top + 12 }}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingHorizontal: 20, gap: 20, paddingBottom: 40 + tabBarClearance }}
      >
